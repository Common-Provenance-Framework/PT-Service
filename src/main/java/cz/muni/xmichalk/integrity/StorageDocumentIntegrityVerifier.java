...
public class StorageDocumentIntegrityVerifier implements IIntegrityVerifier {
    ...
    public boolean verifySignature(String token) {
        try {
            X509Certificate cert = this.getCertificate(token);
            ECPublicKey publicKey = (ECPublicKey) cert.getPublicKey();
            return SignedJWT.parse(token).verify(new ECDSAVerifier(publicKey));
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    private boolean verifyTokenExists(QualifiedName document, String token) {
        String trustedPartyUri = this.getTrustedPartyUri(token);
        String originatorId = this.getOriginatorId(token);
